...
def get_excel_lession():
    ...
    # 设置标题行
    ws.append(['index', 'ID', 'title', 'msgtime'])

    # 设置标题行的字体样式
    font = Font(name='Arial', size=12, bold=True, color='FF0000')  # 设置字体为Arial，大小为12，粗体，红色
    for cell in ws[1]:
        cell.font = font

    # 添加数据行
    for row in data:
        ws.append([row['index'], row['ID'], row['title'], row['msgtime']])

    # 保存工作簿
    wb.save('fancy_excel.xlsx')
